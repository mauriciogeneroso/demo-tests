...
public record GrafanaFieldDto(@JsonProperty("field_name") String fieldName,
    ...
    public static class NodeFieldsBuilder {
        ...
        public NodeFieldsBuilder withDescription() {
        ...
        public List<GrafanaFieldDto> build() {
            ...
            if (title) {
                nodeFields.add(new GrafanaFieldDto("title", STRING.getName()));
            }

            if (subTitle) {
                nodeFields.add(new GrafanaFieldDto("subtitle", STRING.getName()));
            }

            if (mainStat) {
                nodeFields.add(new GrafanaFieldDto("mainstat", STRING.getName()));
            }

            if (secondaryStat) {
                nodeFields.add(new GrafanaFieldDto("secondarystat", NUMBER.getName()));
            }

            if (color) {
                nodeFields.add(new GrafanaFieldDto("arc__failed", NUMBER.getName(), null, "red"));
                nodeFields.add(new GrafanaFieldDto("arc__passed", NUMBER.getName(), null, "green"));
            }

            if (description) {
                nodeFields.add(new GrafanaFieldDto("detail__description", STRING.getName()));
            }
